...
假设输入序列分别为 $x_i$ 与 $y_i$. 本函数计算两个序列的离散卷积，即
$$
...
假定输入序列为 $z_i$ 与 $y_i$. 本函数计算满足如下形式的去卷积
$$
...
在小波分析中，离散小波变换（DWT）将信号分解为一组相互正交的小波基函数。这些函数与正弦基函数（傅里叶变换中使用的）的不同之处在于，它们是空间局部化的，即仅在总信号长度的一部分上非零。此外，小波函数是母小波的放大、平移和缩放版本。和傅里叶分析一样，离散小波变换是可逆的。

1维的离散小波变换将信号分解为经过高通滤波器的高频信号与经过低通滤波器的低频信号。因此，离散小波变换近似于一个带通滤波器，使频率与小波相似的信号通过。如果变换的次数大于 1，那么函数将迭代分解低通信号。这将满足在低频部分频域细分，高频部分时域细分的要求。

推导滤波器系数十分复杂。本函数中，小波系数就是离散小波变换中高通滤波器的滤波器系数。函数已经内置了一些经典的小波系数。
...
在DWT函数中已经介绍过，离散小波变换是可逆的。本函数将分解后的信号还原为原始序列。可以先使用 DWT，移除噪声信号后，再使用IDWT，以实现降噪的目的。
